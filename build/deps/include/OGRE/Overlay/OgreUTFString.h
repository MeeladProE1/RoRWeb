...
// libc++ (used by Emscripten/WebAssembly builds) does not provide
// std::char_traits<unsigned short>, while OgreUTFString stores its data in
// std::basic_string<unsigned short>. MSVC and libstdc++ provide it; here we
// add the missing specialization for libc++-based toolchains.
#if OGRE_COMPILER != OGRE_COMPILER_MSVC && defined(__EMSCRIPTEN__) && !defined(CHAR_TRAITS_WEB_SPECIALIZED)
...
namespace std
{
    // Shared implementation for the char_traits specializations below.
    template<typename T>
    ...
    template<>
    struct char_traits<unsigned short> : basic_char_traits<unsigned short> {};

    template<>
    struct char_traits<unsigned int> : basic_char_traits<unsigned int> {};
}
#endif // OGRE_COMPILER != OGRE_COMPILER_MSVC && defined(__EMSCRIPTEN__) && !defined(CHAR_TRAITS_WEB_SPECIALIZED)
