...
namespace
{
    // WebGL has no fixed-function pipeline, so the skybox material needs explicit
    // GLSL shaders. The skybox renders a box centered on the camera: the vertex
    // positions are the box corners and the 3D texcoords are the cube-map sampling
    // directions. On desktop (with FFP) these shaders are never installed.
    void EnsureSkyboxShaders(const Ogre::String& materialName)
    {
        ...
        // The material may live in any group (the default sky is in 'MaterialsRG',
        // custom cubemap materials come from mods), so resolve it generically.
        ...
        const String& group = mat->getGroup();
        ...
        const String vp_src =
            ...
            "void main()\n"
            "{\n"
            "    gl_Position = worldviewproj_matrix * vec4(position, 1.0);\n"
            "    vDir = uv0;\n"
            "}\n";
        const String fp_src =
            "#ifdef GL_ES\n"
        ...
        HighLevelGpuProgramPtr vp = mgr.getByName("rigsofrods/SkyboxVP", group);
        if (!vp)
        {
            vp = mgr.createProgram("rigsofrods/SkyboxVP", group, lang, GPT_VERTEX_PROGRAM);
            vp->setSource(vp_src);
            vp->load();
        }

        HighLevelGpuProgramPtr fp = mgr.getByName("rigsofrods/SkyboxFP", group);
        if (!fp)
        {
            fp = mgr.createProgram("rigsofrods/SkyboxFP", group, lang, GPT_FRAGMENT_PROGRAM);
            fp->setSource(fp_src);
            fp->load();
        }
        ...
        // Bind the cube sampler to texture unit 0 (the material's cubic texture unit)
        GpuProgramParametersSharedPtr fpParams = fp->getDefaultParameters();
        fpParams->setIgnoreMissingParams(true);
        fpParams->setNamedConstant("skybox", 0);

        GpuProgramParametersSharedPtr vpParams = vp->getDefaultParameters();
        ...
        vpParams->setNamedAutoConstant("worldviewproj", GpuProgramParameters::ACT_WORLDVIEWPROJ_MATRIX);
    }
}
...
void RoR::Terrain::initSkySubSystem()
{
    ...
    else
    {
        ...
        {
            // use custom
            EnsureSkyboxShaders(m_def->cubemap_config);
            App::GetGfxScene()->GetSceneManager()->setSkyBox(true, m_def->cubemap_config, 100, true);
        }
        else
        {
            // use default
            EnsureSkyboxShaders("tracks/skyboxcol");
