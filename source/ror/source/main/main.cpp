...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        Ogre::TextureManager::getSingleton().createManual ("EnvironmentTexture",
            ...
            // WebGL/GLES2 does not support this project's 24-bit render-target mapping.
            // Use native RGBA for all off-screen targets.
            Ogre::PF_A8B8G8R8, Ogre::TU_RENDERTARGET, 0, false, fsaa);
        ...
        // One iteration of the main rendering/event loop. Returns false when the
        // game has requested shutdown. On Emscripten this is driven by
        // emscripten_set_main_loop() so the browser stays responsive without
        // needing ASYNCIFY; on desktop it is simply looped.
        static const auto runOneFrame = []() -> bool
        {
            ...
            // Check FPS limit (the browser's rAF already caps the frame rate)
            ...
            return (App::app_state->getEnum<AppState>() != AppState::SHUTDOWN);
        }; // End of main rendering/input loop body

#ifdef __EMSCRIPTEN__
        // Drive one frame per browser animation frame. runOneFrame has static
        // storage and no captures, so it stays valid after main() returns.
