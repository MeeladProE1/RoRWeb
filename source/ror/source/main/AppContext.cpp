...
// RTSS listener for the web build.
//
// OgreBites' stock SGTechniqueResolverListener only converts fixed-function
// materials (overProgrammable=false). That leaves the game's *managed*
// vehicle materials untouched: they reference Cg shaders, which cannot load
// on WebGL, so the first spawned vehicle would crash with a missing-program
// exception. This listener additionally converts those (overProgrammable=true)
// so RTSS generates FFP-equivalent shaders from their pass state.
// Materials with real GLSL programs (terrain, wallpaper, skybox, ImGui) are
// never converted — they keep their custom shaders.
class WebRTSSListener : public Ogre::MaterialManager::Listener
{
public:
    explicit WebRTSSListener(Ogre::RTShader::ShaderGenerator* sg) : m_sg(sg) {}
        ...
        unsigned short lodIndex, const Ogre::Renderable* rend) override
    {
        if (schemeName != Ogre::RTShader::ShaderGenerator::DEFAULT_SCHEME_NAME)
            return nullptr;
        if (!m_sg->hasRenderState(schemeName))
            return nullptr;

        // Managed materials are Cg-based (unloadable on WebGL): let RTSS
        // rebuild them from their FFP pass state.
        const bool over_programmable = originalMaterial->getName().find("managed/") == 0;

        if (!m_sg->createShaderBasedTechnique(*originalMaterial,
                Ogre::MaterialManager::DEFAULT_SCHEME_NAME, schemeName, over_programmable))
        {
            return nullptr;
        }

        m_sg->validateMaterial(schemeName, originalMaterial->getName(), originalMaterial->getGroup());
    ...
    {
        if (m_sg->hasRenderState(tech->getSchemeName()))
        {
            Ogre::Material* mat = tech->getParent();
            m_sg->validateMaterialIlluminationPasses(tech->getSchemeName(),
    ...
    bool beforeIlluminationPassesCleared(Ogre::Technique* tech) override
    {
        ...
        {
            Ogre::Material* mat = tech->getParent();
            m_sg->invalidateMaterialIlluminationPasses(tech->getSchemeName(),
                mat->getName(), mat->getGroup());
            return true;
        }
        return false;
...
private:
    Ogre::RTShader::ShaderGenerator* m_sg;
};
#endif
...
#ifdef __EMSCRIPTEN__
    // On Emscripten Ogre::Root::loadPlugin() is a no-op (no dynamic loading),
    // so the statically-linked plugins (GLES2 render system, ParticleFX,
    // OctreeSceneManager, STBI codec, ...) must be installed explicitly.
    static OgreBites::StaticPluginLoader static_plugin_loader;
    static_plugin_loader.load();
#endif
...
#ifdef __EMSCRIPTEN__
    // WebGL has NO fixed-function pipeline. The RTShader System (RTSS) auto-generates
    // GLSL ES shaders for every fixed-function material (BaseWhite, particles, Hydrax,
    // plain object materials, mods, ...) so they render instead of throwing
    // "RenderSystem does not support FixedFunction". Materials that already have
    // custom shaders (terrain, wallpaper, skybox) are left untouched because RTSS only
    // converts non-programmable techniques.
    if (Ogre::RTShader::ShaderGenerator::initialize())
    {
        Ogre::RTShader::ShaderGenerator* sg = Ogre::RTShader::ShaderGenerator::getSingletonPtr();
        sg->setShaderCachePath("");
        m_rtss_listener = new WebRTSSListener(sg);
        Ogre::MaterialManager::getSingleton().addListener(m_rtss_listener);
        // Render every material through the RTSS scheme; the listener generates a
        // shader technique for FFP materials on demand and programmable materials
        // fall back to their own techniques.
        Ogre::MaterialManager::getSingleton().setActiveScheme(
            Ogre::RTShader::ShaderGenerator::DEFAULT_SCHEME_NAME);

        // RTSS-generated GLSL programs #include the RTShaderLib files
        // (FFPLib_*.glsl etc.); make rtshader.zip visible in the OgreInternal
        // group where those includes are resolved.
        {
            Ogre::ResourceGroupManager& rgm = Ogre::ResourceGroupManager::getSingleton();
            const Ogre::String rtshader_zip =
                PathCombine(App::sys_resources_dir->getStr(), "rtshader") + ".zip";
            if (FileExists(rtshader_zip))
            {
                rgm.addResourceLocation(rtshader_zip, "Zip", Ogre::RGN_INTERNAL);
                rgm.initialiseResourceGroup(Ogre::RGN_INTERNAL);
                LOG("[RoR|Startup|Rendering] RTSS shader library registered (rtshader.zip in OgreInternal)");
            }
            else
            {
                LOG("[RoR|Startup|Rendering] WARNING - rtshader.zip not found, RTSS shader generation may fail");
            }
        }

        LOG("[RoR|Startup|Rendering] RTSS initialized: auto-generating shaders for fixed-function materials");
    }
    else
    {
        LOG("[RoR|Startup|Rendering] WARNING - RTSS initialization failed, FFP materials may not render");
    }
#endif
