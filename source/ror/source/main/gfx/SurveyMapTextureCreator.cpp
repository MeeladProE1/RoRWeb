...
{
    // NOTE (web port): PF_R8G8B8 maps to GL_NONE on the GLES2 render system
    // (little-endian) -> invalid texture on WebGL, breaks the survey map.
    // PF_A8B8G8R8 is native little-endian RGBA and works on GL/GLES/WebGL.
    mTexture = Ogre::TextureManager::getSingleton().createManual(mTextureName,
        ...
        Ogre::TU_RENDERTARGET, Ogre::PF_A8B8G8R8, Ogre::TU_RENDERTARGET, 0, false, fsaa);
