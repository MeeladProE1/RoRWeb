...
{
    // NOTE (web port): PF_R8G8B8 is GL_NONE on GLES2 (little-endian) -> the
    // dashboard render target would be invalid on WebGL. PF_A8B8G8R8 is native
    // little-endian RGBA and works on GL/GLES/WebGL.
    m_texture = Ogre::TextureManager::getSingleton().createManual(
        tex_name, rg_name, Ogre::TEX_TYPE_2D, 1024, 512, 0, Ogre::PF_A8B8G8R8, Ogre::TU_RENDERTARGET);
