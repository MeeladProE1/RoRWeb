...
namespace
{
    // WebGL has no fixed-function pipeline, so the ImGui overlay material needs
    // explicit GLSL shaders. On desktop (GL3+ / FFP) these are harmless.
    void SetupImGuiShaders(Pass* pass)
    {
        ...
        // The renderable applies an ortho matrix as its world transform and uses
        // identity view/projection, so gl_Position = world_matrix * vertex.
        const String vp_src =
            ...
            "void main()\n"
            "{\n"
            "    gl_Position = world_matrix * vertex;\n"
            "    vUV0 = uv0;\n"
            "    vColor = colour;\n"
            "}\n";
        const String fp_src =
            ...
            "void main()\n"
            "{\n"
            "    gl_FragColor = texture2D(tex0, vUV0) * vColor;\n"
            "}\n";

        HighLevelGpuProgramPtr vp = mgr.createProgram(
            "ImGui/materialVP", RGN_INTERNAL, lang, GPT_VERTEX_PROGRAM);
        vp->setSource(vp_src);
        vp->load();

        HighLevelGpuProgramPtr fp = mgr.createProgram(
            "ImGui/materialFP", RGN_INTERNAL, lang, GPT_FRAGMENT_PROGRAM);
        fp->setSource(fp_src);
        fp->load();

        pass->setVertexProgram("ImGui/materialVP");
        pass->setFragmentProgram("ImGui/materialFP");
        pass->getVertexProgramParameters()->setNamedAutoConstant(
            "world_matrix", GpuProgramParameters::ACT_WORLD_MATRIX);
    }
}
