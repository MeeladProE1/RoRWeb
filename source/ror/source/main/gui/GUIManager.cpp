...
namespace
{
    // WebGL has no fixed-function pipeline, so the wallpaper material needs
    // explicit GLSL shaders. On desktop (GL3+ / FFP) these are harmless.
    void SetupWallpaperShaders(Ogre::Pass* pass)
    {
        ...
        // Overlays render with identity projection; vertices are already in NDC.
        const String vp_src =
        ...
        const String fp_src =
            ...
            "void main()\n"
            "{\n"
            "    gl_FragColor = texture2D(tex0, vUV0);\n"
            "}\n";

        HighLevelGpuProgramPtr vp = mgr.createProgram(
            "rigsofrods/WallpaperVP", RGN_DEFAULT, lang, GPT_VERTEX_PROGRAM);
        vp->setSource(vp_src);
        vp->load();

        HighLevelGpuProgramPtr fp = mgr.createProgram(
            "rigsofrods/WallpaperFP", RGN_DEFAULT, lang, GPT_FRAGMENT_PROGRAM);
        fp->setSource(fp_src);
        fp->load();

        pass->setVertexProgram("rigsofrods/WallpaperVP");
        pass->setFragmentProgram("rigsofrods/WallpaperFP");
