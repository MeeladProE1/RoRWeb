...
// Codebuff web port: this header uses std::thread::id but did not include <thread>
// (it was pulled in transitively by other headers on desktop platforms).
#include <thread>
...
private:
    ...
    Ogre::MaterialManager::Listener* m_rtss_listener = nullptr;
#endif
};
